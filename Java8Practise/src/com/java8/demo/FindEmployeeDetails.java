...
public class FindEmployeeDetails {
	public static void main(String args[]) {
		...
		employeeList.add(new EmployeeObject(144, "Magesh Kumar Muniyappa", 28, "Male", "Product Development", 2014, 55500.0));
		employeeList.add(new EmployeeObject(144, "Ravana Kumar", 28, "Male", "Product Development", 2014, 55500.0));
		...
		Entry<Double, List<EmployeeObject>> m = employeeList.stream().sorted(Comparator.comparingDouble(EmployeeObject::getSalary).reversed())
				.collect(Collectors.groupingBy(EmployeeObject::getSalary, LinkedHashMap::new, Collectors.toList())).entrySet()
				.stream().limit(3).min(Map.Entry.comparingByKey()).get();
